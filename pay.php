...
// ============================================
// TIP ANIQLASH — yuqoridagi izohdagi qoidaga ko'ra:
//   SPISANIE                 → CHIQIM (eng ustuvor, aniq chiqim belgisi)
//   ZACHISLENIE/postuplenie  → KIRIM
//   P2P                      → KIRIM
//   SCHET TO UZCARD          → CHIQIM
//   Platezh:                 → CHIQIM
//   Perevod na kartu (P2P yo'q, SCHET TO yo'q) → KIRIM
//   Aks holda                → CHIQIM (xavfsiz standart)
// ============================================
if (preg_match('/SPISANIE/i', $body)) {
    $type = 'debit';
} elseif (preg_match('/ZACHISLENIE|zachisleno|postuplenie/i', $body)) {
    $type = 'credit';
} elseif (preg_match('/\bP2P\b/i', $body)) {
    $type = 'credit';
} elseif (preg_match('/SCHET\s+TO\s+UZCARD/i', $body)) {
    $type = 'debit';
} elseif (preg_match('/Platezh\s*:/i', $body)) {
    $type = 'debit';
...
if ($card_last !== '****') {
    ...
    while ($sh = safe_fetch_assoc($all_shops)) {
...
if ($ins) {
    ...
    if ($type === 'credit') {
        ...
        if ($checkout_res && safe_num_rows($checkout_res) > 0) {
            $crow        = safe_fetch_assoc($checkout_res);
            $checkout_id = $crow['id'];
            $ch_order    = $crow['order'];
            ...
            $srow = safe_fetch_assoc(mysqli_query($connect,
